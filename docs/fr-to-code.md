...
| T-21 | Shows which trainees have/haven't completed each quiz, and who's on at | Phase 1 | Done (slice 10 added the quiz catalog the dashboard enumerates from — GET /api/admin/quizzes, flat list only, no aggregates per CLAUDE.md "basic on purpose") | src/lib/dashboard/quiz-dashboard.ts, src/lib/dashboard/quiz-index.ts, src/app/api/admin/quizzes/route.ts | src/lib/dashboard/quiz-dashboard.test.ts, src/lib/dashboard/quiz-index.test.ts |
